...
---
## v70
- BGM全曲を64kbps(または96kbps) AAC(m4a)に変換
- ループ処理を実施し、継ぎ目を調整

## v69
- 自然音5種を64kbps AAC(m4a)モノラルに変換。通信制限下での再生を想定
- 焚き火：暖炉の音に差し替え、低域を減衰させ質感を調整
- 雨：水たまりに降る雨の音に差し替え
- 波：ボート音の混入していた素材を差し替え
- 滝を追加
- クレジット表記を更新
...
**症状**
自然音の素材配布元の規約に、素材そのものの二次配布を禁じる条項があり、
Webアプリに同梱して配信する形が規約上グレーだった。

**原因**
素材配布サイトの規約が、動画作品への埋め込みを前提に書かれており、
ブラウザが音声ファイルを直接取得するWeb配信の形態を想定していない。
リポジトリに置いた時点でファイルが取得可能な状態になるため、
二次配布に当たると解釈される余地がある。

**変更**
自然音を雨・波・焚き火・小川の4種に絞り、Freesoundで公開されている
CC0 および CC BY 4.0 の素材に差し替えた。いずれも再配布に制限がないため、
同梱して配信する形で問題が生じない。
設定画面に素材名・作者名・ライセンス・出所URLを記載した。
CC BY 4.0 の素材はクレジット表記が条件となるため、同欄がその要件を満たす。
...
**症状**
カウントダウンの表示が飛ぶことがあった（32→30秒など）。

**原因**
`setInterval` は次のtickまでの時間を保証しないため、`TM.remaining` を
毎tick 1ずつ減らす方式では、処理が詰まったぶんだけ表示が実時間からずれる。

**変更**
区間の終了時刻を `TM.endAt` として実時刻（ms）で保持し、
`Math.ceil((TM.endAt - Date.now()) / 1000)` で残り秒を算出する方式に変更した。
tickが遅れても表示は正しい残り秒になる。
曲送りの `trackRemaining` も、経過したtick数（`elapsed`）分だけ減らすようにした。
一時停止からの再開時は `startCountdown` が呼び直され、基準が引き直される。

### 一時停止からの再開時の音量

**症状**
iOSで一時停止から再開したとき、音量が復帰しなかった。

**原因**
v63でBGMの音量制御をWeb Audio APIのGainNodeに移行した際、
`pauseTimer` 内の `currentAudio.volume` への代入が置き換え漏れとなっていた。
iOS Safariは `HTMLMediaElement.volume` への代入を無視する。

**変更**
`bgmGain.gain.value` を経由するように修正した。

### BGM素材の差し替え

**症状**
BGM素材の配布元の規約に、素材そのものの二次配布を禁じる条項があり、
Webアプリに同梱して配信する形が規約上グレーだった。
...
### v66 — ファイル名の統一とキャッシュ対策

**変更**
- `ScriptBGM.js` を `scriptBGM.js` に改名。GitHub Pages は大文字小文字を区別するため、表記を揃えた
- HTMLの `<script>` `<link>` に `?v=` のクエリを追加。ブラウザが番号違いを別ファイルとして扱うため、更新時に確実に取り直される

**備考**
これまで `?v=` は付いておらず、更新の反映は Live Server のキャッシュ無効化ヘッダと手動リロードに依存していた。iPhone で「直したはずなのに動かない」場面があったのは、これが原因の可能性がある。

`?v=` はHTMLに直接書くため、JSの `APP_VERSION` からは制御できない。変更したファイルの番号だけを手で上げる運用になる。将来 Service Worker を導入すれば、この運用から抜けられる。
...
**原因**
バージョン番号を、JSのログ・HTMLの設定画面・コミットメッセージの3箇所に手で書いていた。v41 のログが更新されないまま v53 まで放置される事故が起きていた。

**変更**
- `APP_VERSION` 定数を導入
- 末尾のログをテンプレートリテラルに変更し、定数を参照する形にした
- 設定画面の表示をJSから書き込む形に変更。HTMLには番号を含まない文章のみを残し、JSが動かなくても文章は読めるようにした
---
